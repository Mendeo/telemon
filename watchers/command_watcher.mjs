...
/**
* Функция для предобработки результата выполнения команды. Вызывается, если два последовательных вызова команды дали разный результат.
* @callback pre
...
* @callback trigger
* @param {string} text - Результат работы команды. Если была определена функция pre, то результат работы этой функции.
* @returns {boolean} - true: Сообщить об изменении в ответе команды, false - не сообщать об этом изменении.
 */

/**
* Функция для постобработки результата работы команды (после работы функции trigger).
* @callback post
* @param {string} text - Результат работы команды. Если была определена функция pre, то результат работы этой функции.
* @returns {string} - Обработанный результат работы команды.
 */
 ...
 * @param {number} period - Периодичность вызова команды в мс.
 * @param {pre} [input.pre] - Функция предобработки результата работы команды.
